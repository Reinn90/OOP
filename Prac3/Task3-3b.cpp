...
void displayBoard(int board[][5])
{
	...
	for (int i = 0; i < 5; i++) // loop through the matrix's rows
	{

		cout << i + 1; // display row index identifiers

		for (int j = 0; j < 5; j++) // loop through the matrix's columns
		{
			...
			if (j != 4) // display the two middle vertical dividers
				cout << " |";
		}

		cout << endl;

		/*if (i != 4) // display the two middle horizontal dividers
...
bool isValidMove(int board[][5], int x, int y)
{
...
bool getXOMove(int board[][5], int noOfMoves, int &x, int &y)
{
...
void addMove(int board[][5], int x, int y, int player)
{
...
// Task 3.4
int gameStatus(int board[][5], int &noOfMoves)
{
...
int main()
{
    int r = 15;
    int c = 15;
	int board[5][5]; // initialise board 3x3 matrix
