...
void TicTacToe::getXMove(int player, int &r, int &c)
{
    ...
    do
    {
        ...
        {
            cout << "Player " << playerSymbol << " made a move on " << row + 1 << " " << col + 1 << endl;
        }

        // check if the move
    } while (!isValidMove(player, row, col));
...
// Human player
void TicTacToe::getOMove(int player, int &r, int &c)
{
    ...
    } while (!isValidMove(player, row - 1, col - 1));
...
void TicTacToe::getMove(int player, int &r, int &c)
{
...
void TicTacToe::play()
{
    ...
    while (done == 0) // while the game is not finished yet
    {
        ...
        playerSymbol = (player == 1) ? 'X' : 'O'; // initialise the players, since player is 1, X starts
        ...
        addMove(player, r, c); // display the player move

        noOfMoves++; // number of moves get's closer to the end of the game

        displayBoard(); // update the board

        done = gameStatus(); // determines the winner of if it's a draw
