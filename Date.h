...
public:
    ...
    Date dateAfterMonths(int numberofMonths);
};

Date Date::dateAfterMonths(int numberofMonths)
{

    int temp = month + numberofMonths;

    // reset counter for month at end of year
    while (temp > 12)
    {
        temp = temp - 12;
        year++;
    ...
    // takes into account february days and leap year
    if (endOfMonth(day))
    {
        month = temp;
        day = days[month];
